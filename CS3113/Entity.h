...
public:
    ...
    float       getLerpFactor()            const { return mLerpFactor;            }
    ...
    void setSpriteSheetDimensions(Vector2 newDim) { mSpriteSheetDimensions = newDim;       }
    void setSpeed(int newSpeed)                   { mSpeed              = newSpeed;        }
    void setFrameSpeed(int newSpeed)              { mFrameSpeed         = newSpeed;        }
    void setJumpingPower(float newPower)          { mJumpingPower       = newPower;        }
    void setAngle(float newAngle)                 { mAngle              = newAngle;        }
    void setEntityType(EntityType type)           { mEntityType         = type;            }
    void setAIState(AIState newState)             { mAIState            = newState;        }
    void setAIType(AIType newType)                { mAIType             = newType;         }
    void setLerpFactor(float factor)              { mLerpFactor         = factor;          }
